...
// Returns IDs of articles to exclude (duplicates covering the same event).
// Batches input to avoid token limits; each batch is ≤80 items.
        ...
        messages: [
          {
            role: 'user',
            content:
              '以下是民生新聞清單（格式：序號. [id] 標題）。請找出報導同一事件的重複新聞，每組只保留最重要的一則，其餘列入排除。只輸出要排除的 id JSON 陣列，不要其他文字。若無重複則輸出空陣列 []。\n\n' +
              numbered + '\n\n輸出：',
          },
        ],
        temperature: 0,
